...
class GCN(nn.Module):
    def __init__(
        ...
        self.layers.append(
            GraphConvolution(nfeat, nhid, with_bias=with_bias, batch_norm=batch_norm)
        )
        for i in range(n_layers - 2):
            self.layers.append(
                GraphConvolution(nhid, nhid, with_bias=with_bias, batch_norm=batch_norm)
            )
        self.layers.append(
            GraphConvolution(nhid, nclass, with_bias=with_bias, batch_norm=False)
        )
    ...
    def forward(self, x, adj, dropout=None):
        for i, layer in enumerate(self.layers[:-1]):
            ...
            x = F.dropout(
                x, dropout if dropout else self.dropout, training=self.training
            )
