...
from src.utils.opengsl_utils import get_homophily, scipy_sparse_to_sparse_tensor
import dgl
...
class GT(nn.Module):
    def __init__(
        ...
        self.act = F.relu if not act == "identity" else lambda x: x
        if input_layer:
        ...
        for i in range(n_layers):
            ...
            if i == n_layers - 1 and not self.output_layer:
                ...
                out_hidden = nhid
            self.trans.append(
                TransformerAttentionModule(in_hidden, out_hidden, num_heads, dropout)
            )
            if self.use_norm:
                self.norms_1.append(self.norm_type(in_hidden))
            if self.ff:
                self.ffns.append(
                    FeedForwardModule(in_hidden, hidden_dim_multiplier, dropout, act)
                )
                if self.use_norm:
...
class GatedResidual(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, only_gate=False):
        ...
        self.proj = nn.Sequential(nn.Linear(dim_out * 3, 1, bias=False), nn.Sigmoid())
    ...
    def forward(self, x, res):
        ...
        if (
            self.only_gate
        ):  # This is for Eq (6), a case when normalizaton and non linearity is not used.
        ...
        for i in range(n_layers):
            if i < n_layers - 1:
                self.layers.append(
                    nn.ModuleList(
                        [
                            GraphTransformerAttn(
                                n_hidden,
                                dim_out=int(n_hidden / n_heads),
                                num_heads=n_heads,
                            ),
...
if __name__ == "__main__":
    model = GT(256, 128, 7, 5, 0.5, 0)
    g = dgl.rand_graph(30, 100)
    # print(model)
    embeddings = torch.rand((30, 256))
    mid, x, homo_heads = model(embeddings, g)
    print(mid)
    print(x)
    print(homo_heads)
