...
from src.models.components.gcn_idgl import GCN
from src.models.components.opengsl_encoder import APPNP, GIN
import dgl
    ...
    def forward(self, input, adj, anchor_mp=True):
        ...
        if anchor_mp:
            ...
            node_norm = node_anchor_adj / torch.clamp(
                torch.sum(node_anchor_adj, dim=-2, keepdim=True), min=1e-12
            )
            anchor_norm = node_anchor_adj / torch.clamp(
                torch.sum(node_anchor_adj, dim=-1, keepdim=True), min=1e-12
            )
            output = torch.matmul(
...
class AnchorGCN(nn.Module):
    def __init__(
        ...
        self.layers.append(
            AnchorGCNLayer(nfeat, nhid, with_bias=with_bias, batch_norm=batch_norm)
        )

        for _ in range(n_layers - 2):
            self.layers.append(
                AnchorGCNLayer(nhid, nhid, with_bias=with_bias, batch_norm=batch_norm)
            )

        self.layers.append(
            AnchorGCNLayer(nhid, nclass, with_bias=with_bias, batch_norm=False)
        )
    ...
    def forward(
        ...
        graph_skip_conn,
        first=True,
        first_init_agg_vec=None,
        init_agg_vec=None,
        update_adj_ratio=None,
        dropout=None,
        first_node_anchor_adj=None,
    ):
        if dropout is None:
        ...
        for encoder in self.layers[1:-1]:
            ...
            if not first:
                mid_cur_agg_vec = update_adj_ratio * mid_cur_agg_vec + (
                    1 - update_adj_ratio
                ) * encoder(node_vec, first_node_anchor_adj, anchor_mp=True)
            node_vec = (
                1 - graph_skip_conn
            ) * mid_cur_agg_vec + graph_skip_conn * encoder(
                node_vec, init_adj, anchor_mp=False
            )
            if self.batch_norm:
        ...
        if not first:
            cur_agg_vec = update_adj_ratio * cur_agg_vec + (
                1 - update_adj_ratio
            ) * self.layers[-1](node_vec, first_node_anchor_adj, anchor_mp=True)
        output = (1 - graph_skip_conn) * cur_agg_vec + graph_skip_conn * self.layers[
            -1
        ](node_vec, init_adj, anchor_mp=False)
...
class GraphLearner(nn.Module):
    ...
    def forward(self, context, anchor=None):
        ...
        context_fc = (
            context.unsqueeze(0) * expand_weight_tensor
        )  # (num_pers, num_node, dim)
        ...
        if anchor is None:
            ...
            attention = torch.matmul(context_norm, anchors_norm.transpose(-1, -2)).mean(
                0
            )  # (num_node, num_anchor)
            markoff_value = 0
...
class IDGL(nn.Module):
    def __init__(
        ...
        self.feat_adj_dropout = 0.5
        self.graph_skip_conn = 0.3

        if self.scalable_run:
            self.encoder = AnchorGCN(
                nfeat=nfeat,
                nhid=n_hidden,
                nclass=nclass,
                n_layers=n_layers,
        ...
        self.graph_learner2 = GraphLearner(
            self.nclass if type == "appnp" else self.hidden_size,
            topk=None,
            epsilon=0.3,
            num_pers=1,
        )
    ...
    def learn_graph(
        ...
        if self.scalable_run:
            ...
            #     torch.sum(raw_adj, dim=-1, keepdim=True), min=1e-12
            # )  # 归一化
    ...
    def forward(self, node_features, init_adj=None):
        node_features = F.dropout(
            node_features, self.feat_adj_dropout, training=self.training
        )
...
if __name__ == "__main__":
    model = IDGL(256, 10, 128)
    g = dgl.rand_graph(30, 100)
    # print(model)
    embeddings = torch.rand((30, 256))
    x, adj = model(embeddings, g.adjacency_matrix().to_dense())
    print(x)
    print(adj.size())
