...
class LINKX(nn.Module):
    ...
    def __init__(
        ...
        self.mlpA = MLP(
            num_nodes, hidden_channels, hidden_channels, init_layers_A, dropout=0
        )
        self.mlpX = MLP(
            in_channels, hidden_channels, hidden_channels, init_layers_X, dropout=0
        )
        self.W = nn.Linear(2 * hidden_channels, hidden_channels)
        self.mlp_final = MLP(
            hidden_channels, hidden_channels, out_channels, num_layers, dropout=dropout
        )
...
class GPRGNN(nn.Module):
    ...
    def reset_parameters(self):
        ...
        self.temp.data[self.k] = self.alpha * (1 - self.alpha) ** torch.arange(
            self.K + 1
        )
...
class GAT(nn.Module):
    def __init__(self, n_feat, n_hidden, n_class, n_layers, n_heads, dropout):
        ...
        for i in range(n_layers - 2):
            self.convs.append(
                GATConv(n_hidden * n_heads, n_hidden, heads=n_heads, dropout=dropout)
            )
        self.convs.append(
            (
                GATConv(
