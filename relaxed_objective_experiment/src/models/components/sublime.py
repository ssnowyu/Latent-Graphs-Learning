...
class FGP_learner(nn.Module):
    def __init__(self, features, k, knn_metric, i, sparse):
        ...
        self.Adj = nn.Parameter(
            torch.from_numpy(
                nearest_neighbors_pre_elu(features, self.k, self.knn_metric, self.i)
            )
        )
...
class GraphEncoder(nn.Module):
    def __init__(
        self, nlayers, in_dim, hidden_dim, emb_dim, proj_dim, dropout, sparse, conf=None
    ):
        ...
        if sparse:
            ...
            if conf.model["type"] == "gcn":
                ...
                self.model = GIN(
                    in_dim, hidden_dim, emb_dim, nlayers, conf.model["mlp_layers"]
                )
...
class GCL(nn.Module):
    def __init__(
        ...
        self.encoder = GraphEncoder(
            nlayers, in_dim, hidden_dim, emb_dim, proj_dim, dropout, sparse, conf
        )
    ...
    def forward(self, x, Adj_, branch=None):
        ...
        if self.sparse:
            if branch == "anchor":
                ...
                Adj = Adj_
            Adj.edata["w"] = F.dropout(
                Adj.edata["w"], p=self.dropout_adj, training=self.training
            )
        else:
...
class GCN_SUB(nn.Module):
    def __init__(
        self, nfeat, nhid, nclass, n_layers=5, dropout=0.5, dropout_adj=0.5, sparse=0
    ):
    ...
    def forward(self, x, Adj):
        if self.sparse:
            ...
            Adj.edata["w"] = F.dropout(
                Adj.edata["w"], p=self.dropout_adj_p, training=self.training
            )
        else:
        ...
        if self.sparse:
            ...
            return x.squeeze(1), Adj
        else:
            return self.model((x, Adj, True)), Adj


if __name__ == "__main__":
    model = GCN_SUB(nfeat=256, nhid=128, nclass=1, sparse=1)

    g = dgl.rand_graph(30, 100)
    g.edata["w"] = torch.ones(100)
    # print(model)
    embeddings = torch.rand((30, 256))
    x, adj = model(embeddings, g)
    # print(mid)
    print(x)
    print(adj)
