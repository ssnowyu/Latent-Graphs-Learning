...
class GAUGModule(LightningModule):
    def __init__(
        self,
    ...
    def training_step(self, batch: dgl.DGLHeteroGraph, batch_idx: int) -> STEP_OUTPUT:
        ...
        for g in graphs:
            ...
            logits, _, _ = self.encoder(feat, origin_g.adjacency_matrix().to_dense())
    ...
    def validation_step(self, batch: dgl.DGLHeteroGraph, batch_idx: int) -> STEP_OUTPUT:
        ...
        for g in graphs:
            ...
            feat = origin_g.ndata["feat"]
            origin_g.edata["w"] = torch.ones(origin_g.num_edges(), device=self.device)
            _, _, optimized_adj = self.encoder(
                feat, origin_g.adjacency_matrix().to_dense()
            )
            ...
            # print(f"Optimized_adj: {optimized_adj}")
            # print(f"label_adj: {label_adj}")
